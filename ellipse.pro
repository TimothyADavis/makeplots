...
END
	...
	;----------------------------------------------------------------
        ...
        ;----------------------------------------------------------------
        if (size(/type,r) eq 5) or (size(/type,a) eq 5) then begin
	  pi = !dpi
	  cf = 1.D0
	endif else begin
	  pi = !pi
	...
	if keyword_set(degrees) then cf = 180/pi
...
pro ellipse, a, b, tilt, a1, a2, xx, yy, help=hlp,$
             ...
             device=device, data=data, norm=norm,_extra=_extra
...
for i = 0, n do begin   	; loop thru ellipse.
  ...
  plots, x + xxi, y + yyi, color=clri, linestyle=lstyli, $
    data=data, device=device, norm=norm,_extra=_extra
endfor
